...
function validatePlan(plan) {
  if (!plan || typeof plan !== 'object' || Array.isArray(plan)) throw new Error('Ready plan must be an object.');
...
export function markReadyForApply(queueDir, plan, { dryRun = false, now = new Date().toISOString() } = {}) {
  validatePlan(plan);
  ...
  if (control.repository !== plan.repository || lane.repository !== plan.repository || item.repository !== plan.repository) {
    throw new Error('Ready plan repository does not match queue state.');
  }
  if (control.syncState !== 'synced') throw new Error(`Queue syncState must be synced before ready transition; current=${control.syncState}.`);
  if (control.generationRevision !== plan.expectedGenerationRevision) throw new Error('Stale ready plan: generation revision changed.');
  if (lane.assignmentRevision !== plan.expectedAssignmentRevision || item.assignmentRevision !== plan.expectedAssignmentRevision) {
    throw new Error('Stale ready plan: assignment revision changed.');
  }
  ...
  if (item.claimHolderId !== plan.holderId) throw new Error('Ready transition holder does not own the current claim.');

  if (item.status === 'ready_for_apply' && lane.state === 'ready_for_apply') {
    return { changed: false, taskId: item.taskId };
  }
  if (item.status !== 'working') throw new Error(`Current task is not ready-transitionable; status=${item.status}.`);
  if (lane.state !== 'working') throw new Error(`Current lane is not ready-transitionable; state=${lane.state}.`);
  ...
  try {
    writeJson(state.itemFile, { ...item, status: 'ready_for_apply', updatedAt: now });
    writeJson(state.laneFile, { ...lane, state: 'ready_for_apply', updatedAt: now });
    const validation = validateQueueRoot(queueDir);
    if (validation.status !== 0) {
      throw new Error(`Queue validation failed after ready transition.\n${validation.stdout ?? ''}${validation.stderr ?? ''}`);
    }
  } catch (error) {
...
function runCli() {
  ...
  if (!queueDir || !planFile) {
    console.error('Usage: node tools/work-queues/mark-work-queue-ready.mjs <queue-dir> <ready-plan.json> [--dry-run]');
    process.exit(2);
  }
  try {
    const result = markReadyForApply(queueDir, readJson(planFile), { dryRun });
    if (dryRun) console.log(`Ready transition is valid for ${result.taskId}; no files changed.`);
    else if (!result.changed) console.log(`${result.taskId} is already ready_for_apply.`);
    else console.log(`Marked ${result.taskId} ready_for_apply.`);
  } catch (error) {
